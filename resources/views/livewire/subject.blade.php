<div>
    @if($form['action'] === 'create')
        ...
        <div class="w-100 flex justify-between items-center gap-2 mb-10">
            <div class="mt-[29px]">
                <a wire:navigate href="{{route('programs.subjects')}}" class="bg-slate-900 py-2 px-6 text-white text-sm font-bold rounded-md">Go Back</a>
            </div>
        </div>
                                                ...
                                                @foreach($item['options']['data'] as $option_key => $options)
                                                    @if (array_key_exists('courses', $options))
                                                        <optgroup label="{{$options['name']}}">
                                                            @foreach ($options['courses'] as $courses)
                                                                <option value="{{$courses['id']}}">{{ucwords($courses['name'])}}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    @else
            ...
            <div class="w-100 md:flex justify-end md:gap-3 mt-10 md:mt-0">
                <select wire:ignore.self wire:model.live='select' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 w-full mb-5 md:mb-0">
                    @if(count($data['courses']) > 0)
                        ...
                        @foreach($data['courses'] as $branches)
                            @if (array_key_exists('courses', $branches))
                                <optgroup label="{{$branches['name']}}">
                                    @foreach ($branches['courses'] as $courses)
                                        <option value="{{$courses['id']}}">{{ucwords($courses['name'])}}</option>
                                    @endforeach
                                </optgroup>
                            @elses
