<div>
    @if($form['action'] === 'create')
        <h1 class="text-3xl font-semibold">{{$form['create']['title']}}</h1>
        <p class="text-sm font-medium mt-1 text-slate-900">{{$form['create']['subtitle']}}</p>
        <div class="w-100 flex justify-between items-center gap-2 mb-10">
            <div class="mt-[29px]">
                <a wire:navigate href="{{route('accounts.administrator')}}" class="bg-slate-900 py-2 px-6 text-white text-sm font-bold rounded-md">Go Back</a>
            </div>
        </div>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-[50px]">
                ...
                <form wire:submit="create" class="p-4 md:p-5">
                    <div class="grid gap-4 mb-4 grid-cols-12">
                        @foreach($form['create']['data'] as $key => $item) 
    ...
    @elseif($form['action'] === 'update') 
        <h1 class="text-3xl font-semibold">{{$form['update']['title']}}</h1>
        <p class="text-sm font-medium mt-1 text-slate-900">{{$form['update']['subtitle']}}</p>
        <div class="w-100 flex justify-between items-center gap-2 mb-10">
            <div class="mt-[29px]">
                <a wire:navigate href="{{route('accounts.administrator')}}" class="bg-slate-900 py-2 px-6 text-white text-sm font-bold rounded-md">Go Back</a>
            </div>
        ...
        <div class="m-auto relative max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-[50px]">
                ...
                <form wire:submit="update" class="p-4 md:p-5">
                    <div class="grid gap-4 mb-4 grid-cols-12">
                        @foreach($form['update']['data'] as $key => $item) 
                            @if(in_array($item['type'], ['text', 'email', 'date', 'password']))
                                <div class="{{$item['css']}}">
                                    <label for="{{$key}}" class="block mb-1 font-extrabold text-gray-900 dark:text-white uppercase" style="font-size: 12px">
                                        {{$item['label']}} 
                            ...
                            @elseif(in_array($item['type'], ['file']))
                                <div class="{{$item['css']}}">
                                    ...
                                    @if ($image && !method_exists($image, 'getClientOriginalExtension'))
                                        <img src="{{ asset('storage/images/users/' . $image) }}" class="w-[200px] h-[150px] object-cover object-center rounded-lg">
                                    @elseif(!empty($image) && session()->has('flash') && session('flash')['status'] == 'success')
                                        <img src="{{ asset('storage/images/users/' . $image) }}" class="w-[200px] h-[150px] object-cover object-center rounded-lg">
                                    @endif       
    ...
    @elseif($form['action'] === 'delete')
        <h1 class="text-3xl font-semibold">{{$form['delete']['title']}}</h1>
        <p class="text-sm font-medium mt-1 text-slate-900">{{$form['delete']['subtitle']}}</p>
        <div class="w-100 flex justify-between items-center gap-2 mb-10">
            <div class="mt-[29px]">
                <a wire:navigate href="{{route('accounts.administrator')}}" class="bg-slate-900 py-2 px-6 text-white text-sm font-bold rounded-md">Go Back</a>
            </div>
        ...
        <div class="m-auto relative max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-[50px]">
                ...
                <form wire:submit="delete" class="p-4 md:p-5">
                    <div class="grid gap-4 mb-4 grid-cols-12">
                        @foreach($form['delete']['data'] as $key => $item) 
    ...
    @else
        ...
        <div class="w-100 md:flex justify-between items-center gap-2 mt-5">
            <div>
                    ...
                    <div class="col-span-12 md:col-span-4 bg-slate-100 shadow-lg rounded-lg text-dark relative overflow-hidden">                        
                        <div wire:ignore.self class="absolute z-10 top-5 right-3 text-teal-50">
                            ...
                            <div wire:ignore.self id="drodown" class="dropdown z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                                    <li>
                                        <a wire:navigate href="?action=update&id={{$users->id}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Update</a>
                                    </li>
                                    <li>
                                        <a wire:navigate href="?action=delete&id={{$users->id}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Delete</a>
                                    </li>
                                </ul>
                            </div>             
                        </div>
                        <div class="max-w-sm h-56 bg-slate-800 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 relatives">
                            <div class="p-5 absolute bottom-0 left-0">
                                <h5 class="text-2xl font-black tracking-tight mb-1 text-white uppercase whitespace-break-spaces line-clamp-1">{{ucwords($users->name)}}</h5>
                                <p class="text-xs text-white font-bold line-clamp-2">{{'@' . $users->username . ' | ' . $users->role}}</p>
                                <p class="text-xs text-white font-bold line-clamp-2">{{$users->email}}</p>
                            ...
                            <div class="absolute top-6 border-2 border-sky-500 left-5 rounded-full text-slate-100 backdrop-blur-sm bg-white/30">
                                @if($users->profile_photo_path)
                                    <img src="{{ asset('storage/' . $users->profile_photo_path) }}" class="rounded-full w-[100px] h-[100px]">
                                @else
                                    <img src="https://ui-avatars.com/api/?name={{ $users->name }}&color=7F9CF5&background=EBF4FF" class="rounded-full w-[100px] h-[100px]">
                                @endif
                            </div>
                        </div>
                </div>
